...
from dradis.models import SydroidPlaylistElement, SydroidPlaylist, SydroidMedia
import json
import re
...
def retreive_medias(request):
    ...
    serialized = json.loads(
        json_serializer.serialize(query[nb_from:nb_from+nb_count],
                                  ensure_ascii=False)
    )
    finalist = []
    ...
    raw = {
        ...
        'medias': camelize(finalist),
        'from': nb_from, 'nb': nb_count
    }
...
def current_playlist(request):
    ...
    curpos = SydroidPlaylist.objects.all()[0].curpos

    query = SydroidPlaylistElement.objects.order_by("position")\
        .filter(position__gte=curpos - 3)
    ...
    raw = {
        'countAll': query.count(),
        'stackElements': camelize(finalist)
    }

    response = HttpResponse(json.dumps(raw),
...
def interface(request):
    return render(request, "interface.html", {
        'dradis_api_key': settings.SYDROID_API_KEY
    })


def redirect_to_new_dradis(_):
    return redirect("/")